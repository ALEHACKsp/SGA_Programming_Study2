...
void CsResource::CreateUAV()
{
	...
	//    & : == 이랑 같음
	if (desc.MiscFlags & D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS/*언오더드 뷰랑 같음*/)
	{
...
void CsAppend::Read(OUT void * data)
{
	// dispatch indirect 쓸 때 스레드에 대한 갯수 구하는거
	//D3D::GetDC()->CopyStructureCount()
	D3D::GetDC()->CopyResource(result, buffer);
