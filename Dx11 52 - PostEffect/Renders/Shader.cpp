...
Shader::~Shader()
{
	...
	map<string, EffectBuffer>::iterator iter;
	for (iter = cBufferMap.begin(); iter != cBufferMap.end(); iter++) {
		SAFE_RELEASE(iter->second.Buffer);
	}

	map<string, EffectVariable>::iterator iter2;
	for (iter2 = variableMap.begin(); iter2 != variableMap.end(); iter2++)
		SAFE_RELEASE(iter2->second.Variable);
