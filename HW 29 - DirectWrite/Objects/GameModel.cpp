...
void GameModel::Render2D()
{
	//if (selectBone != -1) {
	//	ModelBone* bone = model->BoneByIndex(selectBone);
	//	wstring boneName = bone->Name();

	//	RECT rect = { 100, 100, 200, 200 };
	//	DirectWrite::Get()->RenderText(boneName, rect);
	//}
}
...
D3DXMATRIX GameModel::SelectBoneMatrix()
{
	D3DXMATRIX matrix;
	D3DXMatrixIdentity(&matrix);

	if (selectBone != -1) 
		matrix = model->BoneByIndex(selectBone)->Global() * boneTransforms[selectBone];
