...
void ModelClip::Save(wstring file, Model* model)
{
	...
	for (int i = 0; i < model->BoneCount(); i++) {
		...
		ModelKeyframe* keyframe = keyframeMap[name];
		w->String(String::ToString(keyframe->boneName));
		w->UInt(keyframe->transforms.size());

		w->Byte(&keyframe->transforms[0], sizeof(ModelKeyframe::Transform) * keyframe->transforms.size());
	}
