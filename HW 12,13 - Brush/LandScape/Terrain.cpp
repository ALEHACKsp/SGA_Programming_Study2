	...
	, fillMode(0), colorMode(0)
{
	...
	alphaTexture->ReadPixels(DXGI_FORMAT_R8G8B8A8_UNORM, &colorMap);
...
void Terrain::PostRender()
{
	...
	ImGui::Separator();
	ImGui::Text("Brush");
	ImGui::SliderInt("Type", &brushBuffer->Data.Type, 0, 3);
	...
	if (ImGui::Button("Save ColorMap"))
		alphaTexture->SaveFile(alphaTexture->GetFile());
...
void Terrain::AdjustColorMap(D3DXVECTOR3 & location, bool isColorDown)
{
	...
	if (brushBuffer->Data.Type == 1)
	{
		for (UINT z = box.bottom; z <= box.top; z++) {
			for (UINT x = box.left; x <= box.right; x++) {
				...
				if (isColorDown && colorMap[index].r > 0.0f)
				{
					colorMap[index].r -= capacity * Time::Delta();
					if (colorMap[index].r < 0)
						colorMap[index].r = 0;
					isColorMapChange = true;
				}
				else if (!isColorDown && colorMap[index].r < 1.0f)
				{
					colorMap[index].r += capacity * Time::Delta();
					if (colorMap[index].r > 1.0f)
						colorMap[index].r = 1.0f;
					isColorMapChange = true;
				}
			}
		}
	...
	if (isColorMapChange)
	{
		alphaTexture->WritePixels(DXGI_FORMAT_R8G8B8A8_UNORM, colorMap);
	}
}
