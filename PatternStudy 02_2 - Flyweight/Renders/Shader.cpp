...
Shader::Shader(wstring shaderFile, string vsName, string psName,
	bool bInstance)
	: shaderFile(shaderFile), vsName(vsName), psName(psName), bInstance(bInstance)
{
	CreateVertexShader();
	CreatePixelShader();
	CreateInputLayout();
...
void Shader::CreateInputLayout()
{
	...
	for (UINT i = 0; i< shaderDesc.InputParameters; i++)
	{
		...
		if (bInstance && temp == "INSTANCE")
		{
			// 자료형에 맞게 format 설정 int r32_uint float4 r32g32b32a32 float 이런식
			elementDesc.Format = DXGI_FORMAT_R32_UINT;
