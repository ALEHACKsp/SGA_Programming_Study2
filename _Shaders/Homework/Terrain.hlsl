...
float3 Line(float3 oPosition, float spacing, float3 color, inout bool check)
{
    if (check == true)
        return float3(0, 0, 0);
    ...
    if (LineType == 1)
    {
        ...
        grid.x = frac(oPosition.x / spacing); // spacing 으로 나눈 건 0~1 사이로 만들려고
        grid.y = frac(oPosition.z / spacing); // spacing 으로 나눈 건 0~1 사이로 만들려고
    ...
    {
        float2 grid = oPosition.xz / spacing;
        ...
        return lerp(color, float3(0, 0, 0), weights);
    }
...
float4 PS(PixelInput input) : SV_TARGET
{
    ...
    color = color + float4(Line(input.oPosition, 128, float3(1, 1, 1), check), 0);
    color = color + float4(Line(input.oPosition, 64, float3(1, 0, 0), check), 0);
    color = color + float4(Line(input.oPosition, 32, float3(0, 1, 0), check), 0);
    color = color + float4(Line(input.oPosition, 16, float3(0, 0, 1), check), 0);
