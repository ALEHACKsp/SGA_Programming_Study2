...
void GameModel::PostRender()
{
	...
	string label = (String::ToString(name) + " Pos");
	ImGui::DragFloat3(label.c_str(), fPos, 0.01f);
	Position(fPos);

	D3DXVECTOR3 scale = Scale();
	float fScale[] = { scale.x, scale.y, scale.z };
	label = (String::ToString(name) + " Scale");
	ImGui::DragFloat3(label.c_str(), fScale, 0.01f);
	Scale(fScale);
}
...
{
	vector<Material*> mats = this->model->Materials();
