...
void QuadTree::CreateTreeNode(NodeType * node, float positionX, float positionZ, float width)
{
	...
	// Box 생성
	UINT num = (UINT)floor((float)triangles / (float)DivideCount + 0.5f);
	UINT level = 0;
	while ((num = num >> 1))
		++level;
	if (level < 4)
	{
		...
		D3DXVECTOR3 minPos(positionX - d, 0.0f, positionZ - d);
		D3DXVECTOR3 maxPos(positionX + d, 10.0f + level, positionZ + d);
