...
void TerrainRender::Initialize()
{
	...
	tempRtv = new RenderTargetView(
		terrain->GetHeightMap()->Width(),
		terrain->GetHeightMap()->Height(), DXGI_FORMAT_R16_FLOAT);
}
...
void TerrainRender::AdjustY()
{
	...
	D3D::GetDC()->CopyResource(tempRtv->Texture(), heightMapTex);

	adjustHeightMapComputeShader->AsShaderResource("Input")->SetResource(tempRtv->SRV());
