...
D3DXCOLOR Textures::ReadPixel(ID3D11Texture2D * texture, UINT x, UINT y)
{
	D3D11_BOX box;
	box.left = x;
	...
	hr = D3D::GetDevice()->CreateTexture2D(&desc, NULL, &tempTexture);

	D3D::GetDC()->CopySubresourceRegion(tempTexture, 0, 0, 0, 0, texture, 0, &box);

	D3D11_MAPPED_SUBRESOURCE map;

	D3DXCOLOR pixel;
	D3D::GetDC()->Map(tempTexture, 0, D3D11_MAP_READ, 0, &map);
	memcpy(&pixel, map.pData, sizeof(D3DXCOLOR));
