...
void CsResource::CreateRawBuffer2(UINT size, void * initData, ID3D11Buffer ** buffer)
{
	D3D11_BUFFER_DESC desc = { 0 };
	desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D10_BIND_VERTEX_BUFFER;
	desc.ByteWidth = size;
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS;
...
void CsResource::CreateStructuredBuffer2(UINT size, UINT count, void * initData, ID3D11Buffer ** buffer)
{
	D3D11_BUFFER_DESC desc = { 0 };
...
void CsResource::Read(ID3D11Buffer * buffer, UINT byteWidth, OUT void * data)
{
	ID3D11Buffer* result = CsResource::CreateAndCopyBuffer(buffer);
