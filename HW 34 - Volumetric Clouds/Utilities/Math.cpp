...
{
	D3DXVECTOR3 result;
	Math::Transform(value, rotation, result);
	return result;
}

D3DXVECTOR3 Math::Transform(D3DXVECTOR3 position, D3DXMATRIX matrix)
{
	Math::Transform(position, matrix, position);
	return position;
}

void Math::Transform(D3DXVECTOR3 & position, D3DXMATRIX & matrix, OUT D3DXVECTOR3 & result)
{
	float x = (position.x * matrix._11) + (position.y * matrix._21) + (position.z * matrix._31) + matrix._41;
	float y = (position.x * matrix._12) + (position.y * matrix._22) + (position.z * matrix._32) + matrix._42;
	float z = (position.x * matrix._13) + (position.y * matrix._23) + (position.z * matrix._33) + matrix._43;
...
void Math::Transform(D3DXVECTOR3 & value, D3DXQUATERNION & rotation, OUT D3DXVECTOR3 & result)
{
