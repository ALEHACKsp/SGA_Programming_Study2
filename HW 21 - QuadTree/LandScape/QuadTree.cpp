...
QuadTree::QuadTree(ExecuteValues* values, Terrain* terrain)
	...
	, fWidth(512), fHeight(768)
	, isFrameMode(false), isDebugMode(true)
{
	material = new Material(Shaders + L"Homework/Terrain.hlsl");
	material->SetDiffuseMap(Textures + L"Dirt.png");

	worldBuffer = new WorldBuffer();
	lineBuffer = new LineBuffer();

	perspective = new Perspective(fWidth, fHeight, Math::PI * 0.25f, 0.1f, 1000);
	frustum = new Frustum(values, 500, values->MainCamera, perspective);
...
void QuadTree::Render()
{
	...
	ImGui::Checkbox("Frame Mode", &isFrameMode);
	ImGui::Checkbox("Debug Mode", &isDebugMode);
	ImGui::InputFloat("F Width", &fWidth);
	ImGui::InputFloat("F Height", &fHeight);
	perspective->Set(fWidth, fHeight, Math::PI * 0.25f, 0.1f, 1000);
}
...
void QuadTree::RenderNode(NodeType * node)
{
	...
	if (isDebugMode)
		RenderDebugLine(center, node->Width);
...
void QuadTree::RenderDebugLine(D3DXVECTOR3& center, float& width)
{
	...
	if (width < 128)
	{
		...
		debug->Set(center, halfSize, color);
		debug->Render();
