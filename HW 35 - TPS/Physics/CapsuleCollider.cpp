...
void CapsuleCollider::Update(D3DXMATRIX & world)
{
	...
	//D3DXVec3TransformCoord(&S, &S, &matFinal);
	//D3DXVec3TransformCoord(&R, &R, &matFinal);
	//D3DXVec3TransformCoord(&T, &T, &matFinal);
...
void CapsuleCollider::Get(OUT D3DXVECTOR3 * a, OUT D3DXVECTOR3 * b, OUT float * radius)
{
	...
	if (bBoneWorld) {
		D3DXMATRIX matFinal = World() * boneWorld;
		D3DXVECTOR3 S, T;
		D3DXQUATERNION R;
		D3DXMatrixDecompose(&S, &R, &T, &matFinal);
		D3DXVECTOR3 rotation;
	...
	*b = dir * height * 0.5f;
	...
	if (bBoneWorld)
	{
		D3DXVec3TransformCoord(a, a, &(World() * boneWorld));
		D3DXVec3TransformCoord(b, b, &(World() * boneWorld));
	}
	else {
