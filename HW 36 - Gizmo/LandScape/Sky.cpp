...
Sky::Sky(ExecuteValues * values)
	: values(values)
	, realTime(false), theta((float)D3DX_PI / 2), phi(0), timeSpeed(0.2f)
...
D3DXVECTOR3 Sky::GetSunColor(float theta, int turbidity)
{
	...
	float coseno = cosf(theta + (float)D3DX_PI);
	float factor = theta / (float)D3DX_PI * 180.0f;
...
void Sky::RenderGlow()
{
	...
	D3DXMatrixRotationX(&rotationX, -(theta + (float)D3DX_PI / 2.0f));
	D3DXMatrixRotationY(&rotationY, -(-phi + (float)D3DX_PI / 2.0f));
	...
	if (theta < (float)D3DX_PI / 2.0f || theta > 3.0f * (float)D3DX_PI / 2.0f)
		moonBuffer->Data.Alpha = fabs(sinf(theta + (float)D3DX_PI / 2.0f));
	else
...
void Sky::RenderClouds()
{
	...
	D3DXMatrixRotationX(&rotationX, -(float)D3DX_PI / 2.0f);
	D3DXMatrixTranslation(&translation2, camPos.x, camPos.y, camPos.z);
