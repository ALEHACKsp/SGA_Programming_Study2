...
ExeLine::ExeLine(ExecuteValues * values)
	: Execute(values)
	, vertices(NULL), vertexBuffer(NULL)
{
	shader = new Shader(Shaders + L"003_Color.hlsl");
	worldBuffer = new WorldBuffer();
...
void ExeLine::Render(D3DXVECTOR3 * startPos, D3DXVECTOR3 * endPos, UINT lineCount)
{
...
void ExeLine::Render(D3DXVECTOR3 * positions, UINT lineCount, 
