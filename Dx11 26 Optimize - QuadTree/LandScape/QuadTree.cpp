...
const UINT QuadTree::MinimumDivide = 8;

#define OPTIMIZE 0 // 선언시 최적화 주석시 기존 방식 
...
#else
	if (ContainTriangleCount(positionX, positionZ, width, DivideCount, &triangles))
#endif // !OPTIMIZE
	{
		for (UINT i = 0; i < 4; i++)
		{
...
#else
			UINT count;
			if (ContainTriangleCount((positionX + offsetX), (positionZ + offsetZ), (width / 2.0f),
				0, &count))
#endif // !OPTIMIZE	
			{
...
bool QuadTree::ContainTriangleCount(float positionX, float positionZ, float width, 
	int condition, OUT UINT * result)
{
	if (width < (float)MinimumDivide)
	{
		*result = 0;
		return false;
	}
