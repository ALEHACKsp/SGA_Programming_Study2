...
TestDetailMap::TestDetailMap(ExecuteValues * values)
	:Execute(values)
{
	sky = new Sky(values);

	shader = new Shader(Shaders + L"027_NormalMap.hlsl");
	detailShader = new Shader(Shaders + L"Homework/DetailMap.hlsl");
...
TestDetailMap::~TestDetailMap()
{
	SAFE_DELETE(shader);
	SAFE_DELETE(detailShader);
...
void TestDetailMap::Render()
{

}

void TestDetailMap::PostRender()
{
	D3DDesc desc;
	D3D::GetDesc(&desc);
