...
{
	if (isMemory && vec != position)
	{
		Memory::Do(this, Memory_Position, position);
		isMemory = false;
	}
	position = vec;
...
{
	if (isMemory && vec != scale)
	{
		Memory::Do(this, Memory_Scale, scale);
		isMemory = false;
	}
	scale = vec;
...
{
	if (isMemory && vec != rotation)
	{
		Memory::Do(this, Memory_Rotation, rotation);
		isMemory = false;
	}
	rotation = vec;
